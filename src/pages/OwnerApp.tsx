...
export const Dono = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Podemos armazenar isso em um novo estáado se quisermos, ou calcular no render.

      // Vou usar o estáado 'gastos' que já existe para o filtro do mês atual.
  ...
  const handleUpdateEstoque = async (id: string, current: number, delta: number) => {
    ...
    const { error } = await supabase.from('produtos').update({ estáoque: newValue }).eq('id', id);
  ...
  const handleCopyLink = (qrCode: string) => {
    ...
    alert("Link copiado para o clipboard! ðŸ”—");
  ...
  const handleAddProduto = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.from('produtos').insert({
      ...
      nome: data.nome, categoria: data.categoria, preco: parseFloat(data.preco as string), estáoque: parseInt(data.estáoque as string), ativo: true
          ...
          >
            ...
            <div className="card" style={{ padding: '1.5rem', border: '1px solid var(--primary-color)22' }}>
              ...
              {ordersByPaymentMethod[selectedPaymentDetail]?.length === 0 ? (
                ...
                <div className="text-center py-10 opacity-50">Nenhum pedido encontrado para estáe método.</div>
  ...
  const renderUsuarios = () => (
    ...
    <div className="animate-fade-in">
      ...
      <div className="d-flex justify-between items-center mb-6">
        ...
        <h2 style={{ fontSize: isMobile ? '1.4rem' : '1.8rem', fontWeight: 800 }}>Gestáão de Equipe ({usuarios.length})</h2>
    ...
    <div className="animate-fade-in">
      ...
      <div className="card mb-6">
        ...
        <form onSubmit={handleAddProduto} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
          ...
          <input name="estáoque" type="number" placeholder="Estoque" required className="input-field" />
      ...
      <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
        ...
        <input 
          ...
          placeholder="ðŸ” Buscar no estáoque (nome ou categoria)..." 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                ...
                <div>
                  ...
                  <div className="d-flex items-center gap-2">
                    ...
                    <button onClick={() => handleUpdateEstoque(p.id, p.estáoque, -1)} className="btn-outline" style={{width: '36px', height: '36px', padding: 0}}>-</button>

                    <input 

                      key={p.id + '-estáoque-mob-' + p.estáoque}

                      type="number" 

                      defaultValue={p.estáoque} 

                      onBlur={(e) => handleDirectStockInput(p.id, e.target.value)} 

                      style={{ width: '100%', padding: '8px', background: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '8px', textAlign: 'center', color: p.estáoque < 10 ? 'var(--danger-color)' : 'var(--success-color)', fontWeight: 'bold' }} 
              ...
              <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <td style={{ padding: '1rem' }}>
                  ...
                  <div className="d-flex items-center gap-2">
                    ...
                    <button onClick={() => handleUpdateEstoque(p.id, p.estáoque, -1)} className="btn-outline" style={{width: '28px'}}>-</button>

                    <input 

                      key={p.id + '-estáoque-' + p.estáoque}
                      ...
                      style={{ width: '60px', padding: '4px', background: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: '4px', textAlign: 'center', color: p.estáoque < 10 ? 'var(--danger-color)' : 'var(--success-color)', fontWeight: 'bold' }} 
    ...
    return (
      ...
      <div className="animate-fade-in">
        ...
        <div className="d-flex justify-between items-center mb-6">
           ...
           <button className="btn-primary" onClick={handleImprimirRendimentos} style={{ width: 'auto' }}>ðŸ“„ Baixar PDF Rendimentos</button>
    ...
    return (
      ...
      <div className="animate-fade-in" style={{ paddingBottom: '3rem' }}>
        ...
        <div className="mb-8">
            ...
            <p className="text-muted" style={{ fontSize: isMobile ? '0.8rem' : '1rem' }}>Gestáão de turno atual e histórico organizado por pastas.</p>

        </div>

        {/* Turno Atual / Gestáão em Tempo Real */}
                ...
                <table style={{ width: '100%', fontSize: '0.9rem', minWidth: '600px' }}>
                  ...
                  <tbody>
                    ...
                    {currentMonthGastos.length === 0 ? (
                        ...
                        <tr><td colSpan={6} style={{ padding: '2rem', textAlign: 'center', opacity: 0.5 }}>Nenhum gasto nestáe mês.</td></tr>
      ...
      <aside className="sidebar">
        ...
        <div style={{ margin: '1rem 0', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '1rem' }}>
          ...
          {[
            ...
            { label: 'Cozinha / Bar', path: '/producao', emoji: 'ðŸ³' },

            { label: 'Administrador', path: '/admin', emoji: '⚠️™ï¸' },
